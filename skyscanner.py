...
while i < 2:
    ...
    links = driver.find_elements(By.CLASS_NAME,"BpkCard_bpk-card__Yjk0N PlaceCard_cardContent__ODA0Z")
    ...
    for link in links:
        ...
        html = soup.find("a href=")
        place = soup.find("aria-label=")

        print(html)
        print(place)
        if "查看價格" in link.text:
            print(html.text)
        else:
            print(place.text)
